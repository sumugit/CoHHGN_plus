...
class HyperConv(Module):
    def __init__(self, layers, dataset, emb_size, n_node, n_price, n_categoryBig, n_categoryMiddle, n_gender, n_region):
        ...
        self.mat_vcb = nn.Parameter(torch.Tensor(self.n_node, 1))
        self.mat_vcm = nn.Parameter(torch.Tensor(self.n_node, 1))

        self.mat_pcb = nn.Parameter(torch.Tensor(self.n_price, 1))
        self.mat_pcm = nn.Parameter(torch.Tensor(self.n_price, 1))
        self.mat_pv = nn.Parameter(torch.Tensor(self.n_price, 1))
        self.mat_cbp = nn.Parameter(torch.Tensor(self.n_categoryBig, 1))
        self.mat_cbv = nn.Parameter(torch.Tensor(self.n_categoryBig, 1))
        self.mat_cbcm = nn.Parameter(torch.Tensor(self.n_categoryBig, 1))
        self.mat_cmp = nn.Parameter(torch.Tensor(self.n_categoryMiddle, 1))
        self.mat_cmv = nn.Parameter(torch.Tensor(self.n_categoryMiddle, 1))
        self.mat_cmcb = nn.Parameter(torch.Tensor(self.n_categoryMiddle, 1))
        ...
        self.a_o_g_i = nn.Linear(self.emb_size, self.emb_size)
        ...
        self.a_o_g_p = nn.Linear(self.emb_size, self.emb_size)
        ...
        self.a_o_g_cb = nn.Linear(self.emb_size, self.emb_size)
        self.b_o_gcb1 = nn.Linear(self.emb_size, self.emb_size)
        self.b_o_gcb2 = nn.Linear(self.emb_size, self.emb_size)
        self.b_o_gcb3 = nn.Linear(self.emb_size, self.emb_size)
        
        self.a_o_g_cm = nn.Linear(self.emb_size, self.emb_size)
        self.b_o_gcm1 = nn.Linear(self.emb_size, self.emb_size)
        self.b_o_gcm2 = nn.Linear(self.emb_size, self.emb_size)
        self.b_o_gcm3 = nn.Linear(self.emb_size, self.emb_size)
    ...
    def forward(self, adjacency, adjacency_pv, adjacency_vp, adjacency_pcb, adjacency_cbp,
        ...
        for i in range(self.layers):
            item_embeddings = self.second_emb(
                                        self.b_o_gi_all,
                                        self.b_o_gi_all,
                                        self.b_o_gi_all,
                                        self.b_o_gi_all,
                                        item_emb,
                                        self.first_emb(adjacency_vp, self.mat_vp, pri_emb),
                                        self.first_emb(adjacency_vcb, self.mat_vcb, cateBig_emb),
                                        self.first_emb(adjacency_vcm, self.mat_vcm, cateMiddle_emb)                                                                              
                                        ) + self.get_embedding(adjacency, item_emb)
            
            price_embeddings = self.second_emb(
                                        self.b_o_gp_all,
                                        self.b_o_gp_all,
                                        self.b_o_gp_all,
                                        self.b_o_gp_all,
                                        pri_emb,
                                        self.first_emb(adjacency_pv, self.mat_pv, item_emb),
                                        self.first_emb(adjacency_pcb, self.mat_pcb, cateBig_emb),
                                        self.first_emb(adjacency_pcm, self.mat_pcm, cateMiddle_emb)                                        
                                        )
            
            categoryBig_embeddings =  self.second_emb(
                                        self.b_o_gcb_all,
                                        self.b_o_gcb_all,
                                        self.b_o_gcb_all,
                                        self.b_o_gcb_all,
                                        cateBig_emb,
                                        self.first_emb(adjacency_cbp, self.mat_cbp, pri_emb),
                                        self.first_emb(adjacency_cbv, self.mat_cbv, item_emb),
                                        self.first_emb(adjacency_cbcm, self.mat_cbcm, cateMiddle_emb)                                     
                                        )
            
            categoryMiddle_embeddings =  self.second_emb(
                                        self.b_o_gcm_all,
                                        self.b_o_gcm_all,
                                        self.b_o_gcm_all,
                                        self.b_o_gcm_all,
                                        cateMiddle_emb,
                                        self.first_emb(adjacency_cmp, self.mat_cmp, pri_emb),
                                        self.first_emb(adjacency_cmv, self.mat_cmv, item_emb),
                                        self.first_emb(adjacency_cmcb, self.mat_cmcb, cateBig_emb)                                    
                                        )
    ...
    def first_emb(self, adjacency, mat_v, embedding2):
        values = adjacency.data
        indices = np.vstack((adjacency.row, adjacency.col))
        i = torch.LongTensor(indices)
        v = torch.FloatTensor(values)
    ...
    def second_emb(self, a_o_g, b_o_g1, b_o_g_cateBig, b_o_g_cateMiddle, emb_mat1, emb_e1, emb_e2, emb_e3):
...
class CoHHGN_plus(Module):
    ...
    def get_period_encoding(self, period, d):
        """Function to encode week information into a d-dimensional vector
        """
    ...
    def global_emb(self, alias_inputs, session_item, reversed_sess_item, mask, embedding, adj, num):
        ...
        # Learning co-occurrence relations for hop counts
